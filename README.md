...
**Group auto-join with one-tap approval.** Add the bot to a group and send `/channel_join`. The owner gets a four-button card in a private chat, with a short explanation of each mode: reply to everyone, mention-only, observer, or reject. Every decision — rejections included — is appended to an `auth-log.jsonl` journal, and each connected chat gets a context card in `chats/<id>.md` plus a generated `CHATS.md` registry. No numeric chat IDs, no editing JSON by hand; group messages can never change the allowlist themselves — approval only happens in the owner's private chat.
