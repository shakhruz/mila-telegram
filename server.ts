...
// running our own getUpdates. Losing the session's stdio pipe no longer mutes the
// channel: the daemon keeps polling and the bridge replays the queue from a cursor.
const BRIDGE_DIR = join(STATE_DIR, 'inbound')
const BRIDGE_EVENTS = join(BRIDGE_DIR, 'events.jsonl')
const BRIDGE_CURSOR = join(BRIDGE_DIR, 'cursor')
const RECEIVER_PID = join(STATE_DIR, 'receiver.pid')

function receiverAlive(): boolean {
  try {
    const pid = parseInt(readFileSync(RECEIVER_PID, 'utf8'), 10)
    if (!pid) return false
    process.kill(pid, 0)
    return true
  } catch { return false }
...
async function runBridge(): Promise<void> {
  const { statSync, openSync, readSync, closeSync, writeFileSync: wfs } = await import('node:fs')
  let cursor = 0
  try { cursor = parseInt(readFileSync(BRIDGE_CURSOR, 'utf8'), 10) || 0 } catch {}
  try { botUsername = (await bot.api.getMe()).username } catch {}
  process.stderr.write(`telegram channel: BRIDGE mode (receiver-daemon polling), cursor=${cursor}\n`)
  let draining = false
  const drain = async (): Promise<void> => {
    ...
    try {
      for (;;) {
        let size = 0
        try { size = statSync(BRIDGE_EVENTS).size } catch { break }
        if (size <= cursor) break
        const fd = openSync(BRIDGE_EVENTS, 'r')
        const buf = Buffer.alloc(size - cursor)
        readSync(fd, buf, 0, buf.length, cursor)
        closeSync(fd)
        const chunk = buf.toString('utf8')
        const lastNl = chunk.lastIndexOf('\n')
        if (lastNl < 0) break // incomplete line — wait for the rest to be written
        const complete = chunk.slice(0, lastNl)
        for (const line of complete.split('\n')) {
          if (!line.trim()) continue
          try {
            const ev = JSON.parse(line)
            // The journal is an unauthenticated file seam: replay only the two
            // methods the daemon can emit at all. Otherwise a line in the file
            // becomes a way to invoke an arbitrary method in the session.
            if (ev && BRIDGE_ALLOWED_METHODS.has(ev.method)) {
              await mcp.notification({ method: ev.method, params: ev.params })
            } else if (ev && ev.method) {
              process.stderr.write(`bridge: method not allowed, skipped: ${ev.method}\n`)
            }
          } catch (err) {
            process.stderr.write(`bridge: bad event line skipped: ${err}\n`)
          }
        }
        cursor += Buffer.byteLength(complete + '\n', 'utf8')
        wfs(BRIDGE_CURSOR, String(cursor))
      }
    } finally { draining = false }
  }
  await drain() // replay whatever was missed while the pipe was down
  try {
    const { watch } = await import('node:fs')
    watch(BRIDGE_DIR, () => { void drain() })
  } catch {}
  setInterval(() => { void drain() }, 2000).unref()
  setInterval(() => {
    if (!receiverAlive()) process.stderr.write('bridge: ⚠️ receiver daemon is DOWN — inbound frozen until its supervisor restarts it\n')
  }, 60000).unref()
}

void (async () => {
  if (receiverAlive()) { await runBridge(); return }
  process.stderr.write('telegram channel: receiver-daemon not running — classic polling mode\n')
